require(['jquery', 'base', 'model', 'validate', 'tag'], function ($, base, model, validate, tag) {
    base.init();
    //笔译的
    tag.bind({
        id: 'tag_category_id',
        url: '/shop/index/product/ajaxGetCategory?type=1' + '&id=' + P_ID,
        limit: 1
    });
    //口译的
    tag.bind({
        id: 'tag_category_id_k',
        url: '/shop/index/product/ajaxGetCategory?type=2' + '&id=' + P_ID,
        limit: 1
    });
    //服务领域标签
    tag.bind({
        id: 'tag_field_id',
        url: '/shop/index/product/ajaxGetField' + '?id=' + P_ID,
        limit: 3
    });
    //服务承诺
    tag.bind({
        id: 'tag_service_commitment',
        url: '/shop/index/product/ajaxGetServiceCommitment' + '?id=' + P_ID,
        limit: 6
    });
    ...
    var $form = $('#product_form'),
        ...
        createPriceType = function (type,price_type) {
            var option_str = '<option value="">请选择价格单位</option>';
            $.each(units[type], function (i, v) {
                if(price_type == i){
                    option_str += ('<option value="' + i + '" selected>' + v + '</option>');
                }else{
    ...
    //初始化价格单位
    createPriceType($form.find('.js-type').val(),price_type);
